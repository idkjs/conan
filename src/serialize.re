...
let int = (ppf, n) =>
  if (n >= 0) {
    ...
    Format.fprintf(ppf, "@[<1>(%d)@]", n);
  };

let int32 = (ppf, n) =>
  if (n >= 0l) {
    Format.fprintf(ppf, "%ldl", n);
  } else {
    Format.fprintf(ppf, "@[<1>(%ldl)@]", n);
  };
...
let ptime_span = (ppf, span) => {
  let (d, ps) = Ptime.Span.to_d_ps(span);
...
let iter = (~sep as pp_sep=cut, iter, pp, ppf, v) => {
...
let pair = (pp0, pp1) =>
  parens((using(fst, box(pp0)) ++ comma) ++ using(snd, box(pp1)));
...
module Re_serialize = {
  let rec set = (ppf, cset) => {
    ...
    List.iteri(
      (pos, chr) => Bytes.set(res, pos, Char.chr(chr land 0xff)),
      lst^,
    );
  ...
  and sem = (ppf, (sem, t)) =>
    switch (sem) {
    | `Longest =>
      Format.fprintf(ppf, "@[<2>Re.longest@ @[%a@]@]", parens(view), t)
    | `Shortest =>
      Format.fprintf(ppf, "@[<2>Re.shortest@ @[%a@]@]", parens(view), t)
    | `First =>
      Format.fprintf(ppf, "@[<2>Re.first@ @[%a@]@]", parens(view), t)
    }

  and sem_greedy = (ppf, (kind, t)) =>
    switch (kind) {
    | `Greedy =>
      Format.fprintf(ppf, "@[<2>Re.greedy@ @[%a@]@]", parens(view), t)
    | `Non_greedy =>
      Format.fprintf(ppf, "@[<2>Re.non_greedy@ @[%a@]@]", parens(view), t)
    }

  and group = (ppf, t) =>
    Format.fprintf(ppf, "@[<2>Re.group @[%a@]@]", parens(view), t)

  and no_group = (ppf, t) =>
    Format.fprintf(ppf, "@[<2>Re.no_group @[%a@]@]", parens(view), t)

  and nest = (ppf, t) =>
    Format.fprintf(ppf, "@[<2>Re.nest @[%a@]@]", parens(view), t)

  and case = (ppf, t) =>
    Format.fprintf(ppf, "@[<2>Re.case @[%a@]@]", parens(view), t)
  ...
  and view = ppf =>
    ...
    | [@implicit_arity] Re.View.Repeat(t, i, j) =>
      repeat(ppf, (Re.View.view(t), i, j))
    ...
    | [@implicit_arity] Re.View.Sem(s, t) => sem(ppf, (s, Re.View.view(t)))
    | [@implicit_arity] Re.View.Sem_greedy(kind, t) =>
      sem_greedy(ppf, (kind, Re.View.view(t)))
    ...
    | Re.View.Intersection(ts) =>
      intersection(ppf, List.map(Re.View.view, ts))
    | Re.View.Complement(ts) => complement(ppf, List.map(Re.View.view, ts))
    | [@implicit_arity] Re.View.Difference(ta, tb) =>
      difference(ppf, (Re.View.view(ta), Re.View.view(tb)))
    | [@implicit_arity] Re.View.Pmark(mark, t) =>
      pmark(ppf, (mark, Re.View.view(t)));
};
